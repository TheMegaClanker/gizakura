...
Depth is conveyed through tonal bands, borders, patterns, and **liquid-glass panels** — not generic card drop shadows.
...
6. **Liquid glass (`GlassPanel`)** — sakura-tinted blur, inset highlights, and soft `--glass-shadow` on hero copy, nav, about/contact shells, and team cards. Intentional v3 surface treatment; not flat-card shadow elevation.

### Named Rules
**The Flat Card Rule.** Editorial cards, tags, and outline buttons stay flat — no drop shadows. **`GlassPanel` is the exception:** frosted glass with documented `--glass-shadow` is allowed for v3 shells and interactive panels. Do not add ad-hoc `box-shadow` outside the glass token system.

**The Motion Budget Rule.** Sakura petals animate on the **hero only**. Team parallax runs on **desktop (≥768px) only**. Hero shader uses WebGL when available; otherwise a CSS sakura gradient fallback (also used for `prefers-reduced-motion`).
...
- **Nav links:** `.nav-link` uppercase labels (`0.7rem`, tracking `0.16em`), `--muted` default, hover `--fg`. Active section uses `.nav-link-active` (ink text + accent underline). 44px tap height. Scroll-spy highlights the current band on desktop and mobile.
...
- **v3 placement: hero section only** — other bands rely on diagonal hatch patterns.
- Density: `hero` (12 petals animated / 6 static under reduced motion).
- Reduced motion: static positioned petals, fewer count.
- Petal gradient (hero): `#f2b8c0 → #e88996 → #d46a78`.
...
- **Do** keep editorial cards flat with 1px borders — reserve glass shadows for `GlassPanel` only.

### Don't:
- **Don't** add ad-hoc box shadows outside the `GlassPanel` token system.

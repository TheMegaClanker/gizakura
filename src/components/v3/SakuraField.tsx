...
export function SakuraField({
  ...
  const count = density === "hero" ? 28 : 8;
  const petals = useMemo(
    () => makePetals(count, density === "hero" ? 42 : 7),
    [count, density],
  );
      ...
      {petals.map((p) => (
        <motion.span
          ...
          style={{
            ...
            background:
              density === "hero"
                ? "linear-gradient(135deg, #f2b8c0 0%, #e88996 55%, #d46a78 100%)"
                : "linear-gradient(135deg, #e8c4ca 0%, #d9a0aa 100%)",
